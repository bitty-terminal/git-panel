 ...
 *   - Layer 2 `[tools.git]` slice (accepted, CTX-0425; canonical record in
 *     bitty-plugins-docs `specifications/plugin-reuse-and-providers.md`):
 *     this validator additionally pins the `[tools.git]` table shape and
 *     the quoted `"process.spawn:git"` capability key. Any other `[tools.*]`
 *     table fails closed until its own slice is accepted.
 *
 * Usage:
...
/**
 * Heads that may carry a `:PARAMETER` as a quoted boolean key
 * (`"process.spawn:git" = true`). `fs.read`/`fs.write` are intentionally
 * absent: filesystem requests must use `[[capabilities.filesystem]]`.
 */
const PARAMETERIZED_HEADS = new Set([
  "process.spawn",
  "network.connect",
  "mcp.invoke",
  "agent.memory",
...
function validateCapabilityHead(head, path) {
  ...
  // Parameterized form `family.resource:PARAMETER` (accepted Layer 2 slice:
  // `process.spawn:git`, CTX-0425). The base must be a parameter-carrying
  // head from the closed set; the parameter names one allowlisted program
  // or destination. Bare parameter-requiring heads stay rejected below.
  let base = head;
  const colon = head.indexOf(":");
  const parameterized = colon !== -1;
  if (parameterized) {
    base = head.slice(0, colon);
    const parameter = head.slice(colon + 1);
    if (head.indexOf(":", colon + 1) !== -1) {
      fail(`${path} must carry at most one ':PARAMETER'`);
    }
    if (!PARAMETERIZED_HEADS.has(base)) {
      fail(`${path} must not have a ':PARAMETER'`);
    }
    if (
      parameter.length === 0 ||
      parameter.length > MAX_ID_SEGMENT_LEN ||
      !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(parameter)
    ) {
      fail(
        `${path} parameter must match [a-z0-9]+(-[a-z0-9]+)* (max ${MAX_ID_SEGMENT_LEN})`,
      );
    }
  }
  ...
  if (!parameterized && PARAM_REQUIRED_HEADS.has(base)) {
    fail(
      `${path} requires a ':PARAMETER'; use [[capabilities.filesystem]] for fs requests`,
    );
  }
  if (!CLOSED_CAPABILITY_HEADS.has(base)) {
...
/**
 * Validate the Layer 2 system-CLI reuse declaration.
 *
 * Only the accepted `[tools.git]` slice (CTX-0425) passes: any other tool
 * table fails closed until its own slice is accepted. `required` pins
 * fail-closed activation when the tool is missing or mismatched; `version`
 * pins the version constraint re-checked by `bitty plugin doctor`.
 */
function validateTools(value) {
  const table = requireObject(value, "tools");
  rejectUnknownKeys(table, new Set(["git"]), "tools");
  if (table.git === undefined) {
    fail("tools must declare the accepted [tools.git] table");
  }
  const git = requireObject(table.git, "tools.git");
  rejectUnknownKeys(git, new Set(["required", "version"]), "tools.git");
  if (typeof git.required !== "boolean") {
    fail("tools.git.required must be a boolean");
  }
  validateVersionRequirement(
    requireString(git.version, "tools.git.version"),
    "tools.git.version",
  );
}
