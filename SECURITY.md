...
This document describes how security reports for the `git-panel` repository
...
[GitHub Security Advisory](https://github.com/bitty-terminal/git-panel/security/advisories/new).
...
The plugin requests only `panel.provider`, `panel.create`,
`terminal.semantic-read`, `process.spawn:git` (the allowlisted read-only git
verbs), and a read-only filesystem scope. It contains no credentials, no
install-time execution, and no ambient OS authority. Privacy-relevant behavior
is described in [README.md](README.md); any deviation is a security-relevant
defect.
