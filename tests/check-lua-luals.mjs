/**
 * LuaLS conformance check for the git-panel plugin sources.
 *
 ...
 *
 * - positive: `lua/git-panel/**` must diagnose cleanly, including the
 *   `require("git-panel.*")` module tree.
...
function main() {
  ...
  const temp = mkdtempSync(join(tmpdir(), "git-panel-luals-"));
  const problems = [];
  try {
